...
export default function PlanCTA({ answers }) {
  // naive coverage: percentage of non-missing keys across known fields
  const keys = [
    "icp.segment","problem.pain","market.context","solution.offer","solution.edge","solution.benefits",
    "market.region","market.size","market.competitors","icp.buyer","icp.triggers","gtm.channels","gtm.messaging",
    "gtm.pricing","team.founders","team.capabilities","team.gaps","milestones.done","milestones.next90","milestones.roadmap"
  ];
  const has = (obj, path) => path.split(".").reduce((o,k)=> (o && o[k]!==undefined ? o[k] : undefined), obj);
  const filled = keys.filter(k => {
    const v = has(answers||{}, k);
  ...
  return (
    <aside className="plan-cta" style={{border:"1px solid #e5e7eb", padding:"12px", borderRadius:"8px"}}>
      ...
      <div style={{marginTop:"12px", opacity:0.6}}>
        {/* Payment remains behind flags; CTA is informational */}
